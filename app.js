(function () {
  ...
  function renderFlashcards() {
    ...
    const infoBlocks = [
      ...
      infoBlock("易混淆", card.confusing)
    ].join("");
    const hasExpandedContent = Boolean(card.detailedDesc || infoBlocks);
    document.querySelector("#flashcardsView").innerHTML = `
      <section class="study-stage ${state.flashAnswerVisible ? "is-answer-visible" : ""}">
        <div class="flashcard flashcard-container" data-action="toggle-flash" role="button" tabindex="0" aria-label="点击卡片翻转查看">
          <div class="flashcard-inner ${state.flashAnswerVisible ? "is-flipped" : ""}">
            ...
            <div class="flash-face flash-face-back flashcard-back ${state.isExpanded ? "is-expanded" : "is-collapsed"}" aria-hidden="${state.flashAnswerVisible ? "false" : "true"}">
              <div class="flash-back-header">
                <p class="flash-back-keyword">${escapeHtml(card.term)}</p>
                <button class="flash-back-flip" data-action="flip-back" type="button" aria-label="返回正面">↻</button>
              </div>
              <div class="flash-back-content">
                <p class="flash-core"${hasExpandedContent ? " data-action=\"toggle-flash-detail\"" : ""}>${escapeHtml(card.shortDesc)}</p>
                ${hasExpandedContent ? `
                  <button class="flash-expand-button" data-action="toggle-flash-detail" type="button">👇 点击查看详细解析</button>
                  <div class="flash-expanded-content">
  ...
  function toggleFlashcard() {
    ...
    if (!state.flashAnswerVisible) state.isExpanded = false;
    ...
    back?.classList.toggle("is-expanded", state.isExpanded);
    back?.classList.toggle("is-collapsed", !state.isExpanded);
  }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "toggle-flash") {
      if (state.flashAnswerVisible && event.target.closest(".flashcard-back")) return;
      toggleFlashcard();
    }
    if (action === "flip-back") {
      if (state.flashAnswerVisible) toggleFlashcard();
    }
    if (action === "toggle-flash-detail") {
